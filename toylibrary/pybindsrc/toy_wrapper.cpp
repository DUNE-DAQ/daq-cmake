...
int WindUp(int number) {return ++number;}

int WindDown(int number) {return --number;}

void PlayInts(const std::vector<int>& numbers, bool new_line=false) 
{
    std::string separator = new_line ? "\n" : ",";
    std::stringstream numbers_stream;
    for (uint i=0; i < numbers.size(); ++i) {
        if (i) numbers_stream << separator;
        numbers_stream << numbers.at(i);
    }
    std::cout << numbers_stream.str() << std::endl;
}
...
// With daq_add_python_binding_library(), the shared object is named "_daq_${PROJECT_NAME}_py.so".
PYBIND11_MODULE(_daq_toylibrary_py, top_module) {
    ...
    // Here we are adding argument names, as well as defining a default value for one of the function arguments. The function can be called from python with or without that argument.
